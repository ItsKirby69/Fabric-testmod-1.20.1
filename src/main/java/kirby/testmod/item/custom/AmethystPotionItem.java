...
public class AmethystPotionItem extends Item {
    private static final int MAX_USE_TIME = 32;

    public AmethystPotionItem(Item.Settings settings) {
        super(settings);
    }
    ...
    public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
        ...
        if (playerEntity == null || !playerEntity.getAbilities().creativeMode) {
            if (stack.isEmpty()) {
                return new ItemStack(ModItems.AMETHYST_BOTTLE);
            }

            if (playerEntity != null) {
                playerEntity.getInventory().insertStack(new ItemStack(ModItems.AMETHYST_BOTTLE));
            }
        }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (context.getSide() != Direction.DOWN && blockState.isIn(BlockTags.CONVERTABLE_TO_MUD) && PotionUtil.getPotion(itemStack) == Potions.WATER) {
            ...
            playerEntity.setStackInHand(context.getHand(), ItemUsage.exchangeStack(itemStack, playerEntity, new ItemStack(ModItems.AMETHYST_BOTTLE)));
